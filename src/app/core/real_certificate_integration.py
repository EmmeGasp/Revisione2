...
# - Classi: EnhancedExcelExporter, UnderlyingEvaluationEngine, RealCertificateConfig (MODIFICATA), RealCertificateImporter, IntegratedCertificateSystem
# - Funzioni: create_de000vg6drr5_certificate
# ==========================================================

# ========================================
# File Reale_certificate_integration.py ver 15.0
# Timestamp: 2025-07-04
# - La classe RealCertificateConfig è stata aggiornata per includere tutti i campi
#   della GUI v15.1, risolvendo l'errore di 'unexpected keyword argument'.
# - Tutti i nuovi campi sono ora opzionali per mantenere la retrocompatibilità.
# ========================================
...
# NOTA: Questi import potrebbero dover essere aggiornati dopo lo spostamento del file in /core
try:
    ...
    class DateUtils: pass
    class ExpressCertificate: pass
    class PhoenixCertificate: pass
    class UnifiedCertificateFactory: pass
    class CertificateType: pass
    class Barrier: pass
    class BarrierType: pass
    class CouponSchedule: pass
    class UnifiedRiskAnalyzer: pass
    class UnifiedStressTestEngine: pass
    class UnifiedComplianceChecker: pass
    class UnifiedRiskDashboard: pass
...
# ========================================
# REAL CERTIFICATE IMPORTER & CONFIG
# ========================================

@dataclass
class RealCertificateConfig:
    """
    Configurazione unificata per un certificato reale.
    *** MODIFICATA per includere tutti i campi della GUI v15.1 ***
    Tutti i nuovi campi sono opzionali per garantire la retrocompatibilità.
    """
    ...
    # Campi Sottostanti
    underlying_names: Optional[str] = None # Modificato da underlying_assets
    yahoo_ticker: Optional[str] = None
    underlying_currencies: Optional[str] = None
    underlying_dependency_type: Optional[str] = 'Worst-Of'

    # Campi Tassi e Cedole
    risk_free_rate: float = 0.02
    coupon_rate: Optional[float] = None
    coupon_frequency: Optional[str] = None
    coupon_dates: List[datetime] = field(default_factory=list)
    coupon_rates: List[float] = field(default_factory=list) # Coerenza con GUI

    # Campi Barriere
    coupon_barrier: Optional[float] = None
    coupon_barrier_type: Optional[str] = 'none'
    capital_barrier: Optional[float] = None
    capital_barrier_type: Optional[str] = 'protected'
    note_barriere: Optional[str] = None

    # Campi Caratteristiche
    memory_feature: bool = False
    airbag_feature: bool = False
    airbag_level: Optional[float] = None
    airbag_notes: Optional[str] = None

    # Campi Barriera Dinamica
    ...
    # Campi calcolati (per coerenza)
    period_rate: Optional[float] = None
    period_rate_percent: Optional[float] = None
    annual_equivalent_rate: Optional[float] = None
    ...
    def __post_init__(self):
        """Logica post-inizializzazione per garantire la coerenza dei dati."""
        # Se underlying_names è fornito, popola underlying_assets per retrocompatibilità
        if self.underlying_names and not self.underlying_assets:
            self.underlying_assets = [name.strip() for name in self.underlying_names.split(',')]
