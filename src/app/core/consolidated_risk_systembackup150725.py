...
class UnifiedRiskAnalyzer:
    ...
    def analyze_certificate_risk(self,certificate, num_simulations=10000, seed=42):
        """
        Analizza il rischio di un certificato finanziario tramite simulazione Monte Carlo.
        """
        if seed is not None:
            np.random.seed(seed)

        # Validazione preliminare dei dati del certificato
        if not all([
            certificate.initial_spot_prices,
            certificate.volatility,
            certificate.time_to_maturity is not None,
            certificate.risk_free_rate is not None
        ]):
            logger.error("Dati del certificato insufficienti per l'analisi del rischio.")
            return None

        # Estrazione e preparazione dei parametri
        initial_prices = np.array(certificate.initial_spot_prices)
        volatilities = np.array(certificate.volatility) / 100
        risk_free_rate = certificate.risk_free_rate / 100
        time_to_maturity = certificate.time_to_maturity / 365.25
        dividend_yields = np.array(certificate.dividend_yield) / 100 if certificate.dividend_yield else np.zeros_like(initial_prices)
        correlation_matrix = certificate.correlation_matrix

        # Gestione delle scadenze e delle date di osservazione
        observation_dates = sorted([datetime.strptime(d, '%d/%m/%Y') for d in certificate.payment_dates]) if certificate.payment_dates else []
        
        # Generazione dello schedule delle barriere e degli autocall
        schedule = DateCalculationUtils(certificate, observation_dates)
        if schedule is None or schedule.empty:
            logger.error(f"Impossibile generare lo schedule delle barriere per il certificato {certificate.isin}.")
            return None
        
        # Simulazione dei percorsi dei prezzi dei sottostanti
        num_steps = int(time_to_maturity * 365)
        ...
        payoffs = []
        initial_price_worst_of = initial_prices.min()
        
        # Calcolo dei payoff per ogni simulazione
        for i in range(num_simulations):
            autocalled = False
            
            # 1. Controllo Autocall per ogni data di osservazione
            for _, row in schedule.iterrows():
                obs_date = row['date']
                time_to_obs = (obs_date - datetime.now()).days / 365.25
                
                if time_to_obs <= 0:
                    continue

                step_index = min(int(time_to_obs / dt), num_steps - 1)
                prices_at_obs = simulated_prices[:, step_index, i]
                
                if np.min(prices_at_obs) >= row['autocall_level']:
                    payoff = 1.0 + (row['coupon'] / 100)
            ...
            # 2. Se non c'è stato autocall, calcola il payoff a scadenza
            final_price_worst_of = np.min(simulated_prices[:, -1, i])
            final_barrier_level = schedule['barrier_level'].iloc[-1]
            
            is_breached_at_maturity = final_price_worst_of < final_barrier_level

            if is_breached_at_maturity:
                # La barriera è violata a scadenza
                if getattr(certificate, 'airbag_feature', False):
                    
                    # --- SONDA POTENZIATA per AIRBAG ---
                    print("\n--- SONDA AIRBAG ATTIVA ---")
                    print(f"Prezzo Finale: {final_price_worst_of:.2f}")
                    print(f"Livello Barriera Finale USATO: {final_barrier_level:.2f}")
                    print(f"Prezzo Iniziale di Riferimento: {initial_price_worst_of:.2f}")
                    print(f"--> CALCOLO ESEGUITO: {final_price_worst_of:.2f} / {final_barrier_level:.2f}")
                    print("---------------------------\n")
                    # -----------------------------------

                    redemption_value = final_price_worst_of / final_barrier_level
                else:
                    # Senza Airbag: il calcolo è rispetto allo strike
                    redemption_value = final_price_worst_of / initial_price_worst_of
            else:
                # La barriera non è violata: rimborso del nominale + cedola finale
                final_coupon = schedule['coupon'].iloc[-1]
                redemption_value = 1.0 + (final_coupon / 100 if final_coupon else 0)
            
            payoffs.append(redemption_value)

        # Calcolo delle metriche di rischio
        payoffs = np.array(payoffs)
        fair_value = np.mean(payoffs) * 1000  # Assumendo un nominale di 1000
        
        # Calcolo del VaR (Value at Risk) al 95%
        var_95 = np.percentile(payoffs - 1, 5) * 1000

        logger.info(f"Analisi completata per {certificate.isin}: FV={fair_value:.2f}, VaR 95%={var_95:.2f}")
        ...
        try:
            if isinstance(certificate, (ExpressCertificate, PhoenixCertificate)):
                ...
                if certificate.market_data:
                    ...
                    from app.core.structural_cleanup import MarketData
...
def test_consolidated_risk_system():
    ...
    try:
        ...
        from app.core.unified_certificates import create_sample_express_certificate, create_sample_phoenix_certificate
