...
class EnhancedCertificateDialogV15_1_Corrected:
    ...
    def _create_complete_form_fields_v15_1_corrected(self):
        ...
        ttk.Label(fin_row1_frame, text="(es: 3.5 per 3.5%)", 
        ...
        ttk.Label(fin_row2_frame, text="Tasso Cedola (%):", width=15).pack(side=tk.LEFT)
        ...
        self.fields['coupon_frequency'] = ttk.Combobox(fin_row2_frame, width=12,
                                                     values=['Trimestrale', 'Semestrale', 'Annuale'])
        ...
        # *** NUOVO v15.1 *** - Airbag Feature
        ttk.Label(memory_frame, text="Airbag:", width=15).pack(side=tk.LEFT)
        self.fields['airbag_feature'] = ttk.Combobox(memory_frame, width=12,
                                                   values=['True', 'False'])
        self.fields['airbag_feature'].pack(side=tk.LEFT, padx=(5, 0))
        
        # Airbag level
        airbag_level_frame = ttk.Frame(features_frame)
        airbag_level_frame.pack(fill=tk.X, pady=5)
        
        ttk.Label(airbag_level_frame, text="Livello Airbag (%):", width=15).pack(side=tk.LEFT)
        self.fields['airbag_level'] = ttk.Entry(airbag_level_frame, width=10)
        ...
        self.fields['coupon_barrier_type'] = ttk.Combobox(barrier_row1_frame, width=12,
        ...
        self.fields['capital_barrier_type'] = ttk.Combobox(barrier_row2_frame, width=12,
                                                         values=['protected', 'airbag', 'none'])
        ...
        ttk.Label(ticker_frame, text="Yahoo Ticker:", width=15).pack(side=tk.LEFT)
        self.fields['yahoo_ticker'] = ttk.Entry(ticker_frame, width=15)
        self.fields['yahoo_ticker'].pack(side=tk.LEFT, padx=(5, 20))
        
        ttk.Label(ticker_frame, text="Valuta:", width=10).pack(side=tk.LEFT)
        self.fields['currency'] = ttk.Combobox(ticker_frame, width=8,
    ...
    def _load_existing_data_v15_1_corrected(self):
        ...
        if not self.existing_data:
            ...
            self.fields['risk_free_rate'].insert(0, '3.5')  # Formato percentuale
            self.fields['coupon_rate'].insert(0, '2.5')
        ...
        if isinstance(risk_free_rate, (int, float)):
            ...
            try:
                ...
                risk_free_percentage = 3.5  # Default
        ...
        for field_name, data_key in field_mapping.items():
            if field_name in self.fields and data_key in self.existing_data:
                ...
                if field_name == 'memory_feature' or field_name == 'airbag_feature':
                    value = str(value)
                elif field_name in ['coupon_barrier', 'capital_barrier', 'airbag_level'] and isinstance(value, float):
                    value = value * 100 if value <= 1.0 else value  # Converti a percentuale
        ...
        try:
            ...
            for field_name, widget in self.fields.items():
                try:
                    if hasattr(widget, 'get'):
                        ...
                        if value:
                            ...
                            if field_name == 'risk_free_rate':
                                # *** FIX RISK-FREE RATE v15.1 *** - Salva sempre in formato decimale
                                rf_value = float(value)
                                result_data[field_name] = rf_value / 100  # 3.5 -> 0.035
                                print(f"📊 Risk-Free Rate salvato: {rf_value}% -> {rf_value/100}")
                            elif field_name in ['coupon_rate', 'coupon_barrier', 'capital_barrier', 'airbag_level']:
                                # Converti percentuali in decimali
                                perc_value = float(value)
                                result_data[field_name] = perc_value / 100 if perc_value > 1 else perc_value
                    ...
                    print(f"⚠️ Errore lettura campo {field_name}: {e}")
            
            # Validazioni aggiuntive
            if 'risk_free_rate' in result_data:
                rf_rate = result_data['risk_free_rate']
                if rf_rate < 0 or rf_rate > 0.5:  # 0% - 50%
                    messagebox.showerror("Errore", "Risk-Free Rate deve essere tra 0% e 50%")
                    return
            
            # Validazione coerenza airbag
            if result_data.get('airbag_feature', False):
                if not result_data.get('airbag_level'):
                    messagebox.showerror("Errore", "Specificare livello airbag se feature abilitata")
                    return
            ...
            print(f"❌ Errore salvataggio v15.1 CORRECTED: {e}")
            import traceback
            traceback.print_exc()
...
Tasso Cedola: {format_percentage(cert_data.get('coupon_rate', 0))}
...
SOTTOSTANTI:
Yahoo Ticker: {cert_data.get('yahoo_ticker', 'N/A')}
Valuta: {cert_data.get('currency', 'N/A')}
...
"""
    ...
    def _load_certificates(self):
        """*** CARICAMENTO CERTIFICATI v15.1 CORRECTED *** - Con conversione campi v15.1"""
        
        if not self.cert_file.exists():
            print(f"ℹ️ File {self.cert_file} non esiste, creando nuovo")
            return {}
            ...
            for cert_id, cert_data in data.items():
                try:
                    if 'base_config' in cert_data:
                        # Formato enhanced
                        base_config = cert_data['base_config']
                    else:
                        # Formato basic
                        base_config = cert_data
                    
                    # *** CONVERSIONE v15.1 CORRECTED ***
                    # Converte campi v15.1 nel formato compatibile se necessario
                    if any(field in base_config for field in ['coupon_barrier_type', 'capital_barrier_type']):
                        print(f"🔄 Conversione v15.1 per {cert_id}")
                        converted_config = self._convert_v15_1_fields(base_config)
                        certificates[cert_id] = converted_config
                    else:
                        certificates[cert_id] = base_config
